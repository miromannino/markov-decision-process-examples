...
class MDPGUI:
    ...
    def debugCallBack(self, policy, isEnded):
        try:
            if isEnded and self.computationStarted:
                ...
                self.turboModeCheck.config(state=NORMAL)
            ...
            if self.computationStarted:
                ...
                self.c.update()
                time.sleep(int(self.eSleep.get()))
                return True

        except Exception:  # when the window is closed
            return False
    ...
    def toggleComputation(self):
        if not self.computationStarted:
            ...
            self.turboModeCheck.config(state=DISABLED)
            for b in self.radioBAlgorithms: b.config(state=DISABLED)
            if self.algorithm.get() == "vi":
                self.p.valueIteration(self.debugCallBack, self.turboMode.get())
            else:
                self.p.policyIteration(self.debugCallBack, self.turboMode.get())
        else:
            self.bComputation.config(text="Start Computation")
            self.bResetResults.config(state=NORMAL)
            self.turboModeCheck.config(state=NORMAL)
    ...
    def __init__(self, world):
        ...
        for text, mode in (("Value iteration", "vi"), ("Policy iteration", "pi")):
            b = Radiobutton(self.frameComputation, text=text, variable=self.algorithm, value=mode)
        ...
        self.turboModeCheck = Checkbutton(self.frameComputation, text="Turbo fix point", variable=self.turboMode)
...
class MDPChooser:
    ...
    def __init__(self):
        ...
        label = Label(self.master, text="Other rew", bg="gray")
        label.pack(side=TOP, pady=10)
